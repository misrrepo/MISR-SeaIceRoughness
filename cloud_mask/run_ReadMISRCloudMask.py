...
# set the input path
in_dir = "/data/gpfs/assoc/misr_roughness/2016/cloud_masks/april_2016_rccm_9cams/rccm_df"		    # makes a list of all cloudmask.hdf files in here
exe_dir = "/data/gpfs/home/emosadegh/MISR-SeaIceRoughness/exe_dir"			 							# where exe file is on HPC

cloudmask_filetype = "RCCM" #"ASCM" # "SDCM" # 'ASCM' 
...
for file_count, file in enumerate(files_list):
	...
	for block in range(1, end_block_not_included, 1):  # define range for blocks
		...
		print("cmd ret value from C: %d \n" %return_value_of_cmd)
		
		# if (os.system(cmd) != 0):  # old
		if (return_value_of_cmd != 0): # from C, 0==success & 1==error
