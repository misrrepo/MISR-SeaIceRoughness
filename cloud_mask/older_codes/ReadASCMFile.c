...
int readASCMFile(char *fname) 
{
	...
	***/
	...
	// cmask0 = (uint8 *) malloc(128 * 512 * 4 * 4 * sizeof(uint8)); // allocates mem- and returns a ptr to the 1st byte in the allocated mem- == cmask0
	cmask0 = (uint8_t *) malloc(128 * 512 * 4 * 4 * sizeof(uint8_t)); // allocates mem- and returns a ptr to the 1st byte in the allocated mem- == cmask0

	if (!cmask0) // check if ptr is NULLs
	{
		fprintf(stderr, "readASCMFile: malloc failed (cmask0 == 1st byte of allocated mem-)!!!\n");
		return 0;
	}
			...
			for (k = 0; k < 4; k++)
				for (l = 0; l < 4; l++) {
					...
					if ((fraction_cloudBestEst_buf[(j * 4 + k) * 512*4 + i*4 + l] >= 0.0) && (fraction_cloudBestEst_buf[(j * 4 + k) * 512*4 + i*4 + l] < cfcbe_thresh)) { // E: means if upto 0.1 we can take that as not cloudy?
						...
						// cmask0[(j * 4 + k) * 512*4 + i*4 + l] = mask ; // E: original; so cmask0 is either 0 or 1, 0==cloudy 1==clear
						cmask0[(j * 4 + k) * 512*4 + i*4 + l] = 1 ; // E: mine; so cmask0 is either 0 or 1, 0==cloudy 1==clear ; E: replaced mask w/ 1 to test
					...
					// Ehsan: 
						...
						// cmask0[(j * 4 + k) * 512*4 + i*4 + l] = NODATA2 ;
						cmask0[(j * 4 + k) * 512*4 + i*4 + l] = mask ; // E: changed it to test
					}
	...
	{
		fprintf(stderr, "readASCMFile: cmask0 fewer than 65,536 valid in %s: %d\n", fieldName, n);
		return 0;
	}
...
int main(int argc, char *argv[]) {
	...
	if (!write_data(fname[1], cmask0, 512, 2048)) return 1; // E- we only write cmask0 data as output! all elemenst are checked to be total of 512*2048
