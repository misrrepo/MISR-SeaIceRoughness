...
about:
	reads each hdf (MISR imagery) file, transforms TOA-rad to TOA-refl, extracts data for each block from each hdf file, and write out data to each block.
	each hdf file includes nominally 180 blocks in a file.

how to use:
	start with c-code:
	built exe file from c-code source and set the dir path to exe 
	then:
	update lines in the "directory path setting" section
	update dir path for input data to point to where ellipsoid.hdf files are
	update output path
	set the output file labels based on month/day of files downloaded
...
input_dir_fullpath = "/Users/ehsanmos/MLP_dataset/cloudmask_lab/prediction_test_cloudmask_p149_o88247_b10_july_forPaper/tutorial_sampleFile/hdf_files"


#-- where toa-refl.dat files will go
output_path = "/Users/ehsanmos/MLP_dataset/cloudmask_lab/prediction_test_cloudmask_p149_o88247_b10_july_forPaper/tutorial_sampleFile/toa_refl"


exe_dir = "/Users/ehsanmos/MLP_dataset/cloudmask_lab/prediction_test_cloudmask_p149_o88247_b10_july_forPaper/tutorial_sampleFile/exe_dir"
	...
	for cam in camera_list:
		...
		if (not os.path.isdir(cam_dir)):
			...
			if (not os.path.isdir(cam_dir)):
				...
				print('Successfully created camera dir for: %s' % cam)
		else:
