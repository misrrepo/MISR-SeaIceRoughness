...
if (n > 0) // if n positive
	...
	// E- turn off to include blocks= 1-6
...
if (argc < 5) // this might happen later, cos I turnedoff fname[2]
	{
	fprintf(stderr, "Usage: TOARad2Blocks_noblokcs1to6, MISR_GRP_ELLIPSOID_file, block, band, minnaert, toa_data_file \n"); // updated
...
ZOOM = 64;
// for band !=3
if ((band != 2) && (camera != 4)) // if not nadir==vertical camera... E: should we check camera too? or just band is enough?
    {
