...
single_csv_ds_label = "9cam3bands_withDateInfo"
...
column_names = ['path','orbit','block','line','sample','lat','lon','Da_r','Ca_r','Ba_r','Aa_r','An_r','An_g','An_b','An_nir','Af_r','Bf_r','Cf_r','Df_r','mean_ATM_roughness','ATM_date']
...
for atm_cntr, ATMfile in enumerate(atm_list):
	...
	# change time format to Z-format
...
###############################################################
	...
	orbit_path = mtk.time_to_orbit_path(ATM_start_time) #update ZtimeFormat to include HMS to find exact path nu,ber 
	misr_orbit = orbit_path[0]
	misr_path_num = orbit_path[1]
	print(misr_path_num)
...
###############################################################
	...
	ATMfile_total_rows = input_atm_file.shape[0]
	# print(ATMfile_total_rows)


	# read each row of ATM file
	for atm_row_num in range(ATMfile_total_rows): 
...
###########################################################################################################
		...
		misr_orbit_str = str(misr_orbit_int).zfill(6) 
		...
		#### create fulll path to 9 cameras and 3 other AN spectral bands
		...
		for maskedTOA_rawfile in masked_toaFile_orderedList:
			...
			# if a camera is missing, assign fill value for missing cameras
			if (is_file==False):
		...
		final_ds_values = [misr_path_num, misr_orbit, misr_block, int(misr_pixel_x), int(misr_pixel_y), round(atm_lat,7), round(atm_lon,7), pixel_values[0],pixel_values[1],pixel_values[2],pixel_values[3],pixel_values[4],pixel_values[5],pixel_values[6],pixel_values[7],pixel_values[8],pixel_values[9],pixel_values[10],pixel_values[11],atm_roughness,ATM_start_time]
