...
column_names = ['path','orbit','block','line','sample','lat','lon',\
				'Da_r','Ca_r','Ba_r','Aa_r','An_r','An_g','An_b','An_nir','Af_r','Bf_r','Cf_r','Df_r',\
...
# open each ATM file and read each row and will eventually write out a train dataset for each ATM file
for atm_cntr, ATMfile in enumerate(atm_list):
	...
	for misr_orbit_today in orbit_list_today:
		...
		for atm_row_num in range(csv_total_rows): 
			...
			if misr_pathNumber_today not in paths_cover_ATM_location:
				...
				##-- check if all 12 files/cameras are available
				...
				for maskedTOA_rawfile in masked_toaFile_orderedList:
					...
					#-- if one camera is missing, assign fill value for that camera that was not found
					if (is_file==False):
	...
	##################################################################
	# write out the train dataset for each ATM file 

	if (len(final_ds_list) == 0):
		print(final_ds_list)
		print("ATM list is empty, we write -998 flag & continue to next ATM file")
		print("note: -998 means we did not find any MISR pixel that covers ATM locations in that file.")

		
		final_ds_list = [-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998,-998]#,-998,-998]
